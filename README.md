...
2. Skeleton Code
   * npx
   * create-react-app
   * Structure of the demo project that is created
   * New Components should be created as only functions - New components should be developed as functions. All components are functions since React v16, but class components do not need to be redesigned as functions.
3. Introduction
   * We write our React code in a language developed by Facebook known as `JSX - JavaScript Extension`
   * Why should React Components be written in JSX?
      1. It integrates JavaScript and HTML
      2. It is less vulnerable to security threats than JS.
      3. JSX allows you to switch between JS and HTML easily
      4. Demo: *`01-First`, `01-Second`, `whyJSX.jsx`*
   * Demo: Export & `npm start`
   * Error if the component isn't imported - The default presumption is that it is a `.jsx` extension so you don't need to include this in your import statement.
4. Parent/Child Components
    * Create a standard HTML webpage with header, nav, main and footer `(Standard.js)`
    * As it is, the code is **inefficient** and REACT's purpose is to have **reuasable component**
    * Draw the diagram of hierarchy / `App` -> `MyInfo` -> `AwesomeHeader` -> `AwesomeFooter`
    * Difference between *Component* and *element* (i.e. Footer  (Component) footer (Element))
    * Create a footer element (With import and export) then show them how to add to the existing HTML
    * A react Component should do one thing - **single responsibility**
5. Component Hierarchy
   * We should build a wireframe/ mockup to know what components we would need in order to build the application
   * Show Community Example
6. Props
   ...
   * Show youtube page and the simalarities in tiles - how the images are the same, the length of the video, the title, the number of views & date posted etc. - It will be a single component that is developed onces, but we need to make it so that it can change depending on the image or the title.  
   * Demo of what props are and how they're passed
   * Receiving and using props
   * Default Props
   * Prop Types
7. State
   * Data that can change
   * Single source of truth for changing data, any component that relies on this data should receive it as props except for the highest common component which is where the state should live.
   * Minimal set of mutable data the application needs
   * When figuring out if something is a state, ask yourself these questions:
...
8. Forms and Events
   * Methods to handle events with React Elements is very similar to handling events on DOM elements
...
10. Working with External Data
    ...
    * Persisting data requires a temporary or permant store
    ...
    * Understand the lifecycle, so we can work with the data at the right point
    ...
    * Hooks
       ...
       6. State hook:
            * `ExampleWithManyStates.jsx` demos useState():
            * Allows the addition of local state into a function component
            * Requires **useState** to be used (this is the hook)
            * Takes an argument of the initial state
            * Returns an array, which has the current state as the first element, and a function to update it as the second element. We deconstruct this array to declare the state, providing the names of the state and the updating function. It's usual for the updating function to have a prefix of 'set' before the states name. States should never be mutated directly - instead using the updator function
            * Can have more than one state hook in a component - should be declared immediately declared in the body of the component.
            * States can have any valid javascript value, including Strings, booleans, numbers, values and objects.
       7. useEffect Hook:
            * useEffect() is a replacement for CDM, CDU, and CWU.
            * It is used to perform side effects on components such as fetching data, subscriptions or manually changing DOM from React Components
            * UseEffect() runs after React flushes changes to DOM - after every render, including the initial one
            * Takes an argument of a callback function to execute when useEffect() is ran
            * Declared inside component's body so it and it's callback has access to props and state
            * We can have more than one effect hook in a callback
            * `Counter.jsx` demos useEffect():
            * Allows us to hook into the life cycle components to perform side effects hence is name
      8. UseEffect best place to make network calls - 
11. Routing in React
    *  Single Page Application - React-router standing routing library for React
    *  Installation : `npm install react-router-dom --save` needs to be added to the project as a dependency
    *  Linking components:
...
12. Data Requests
    ...
    *  GET/ POST 
    *  Demonstrate with API online 
    *  Set the information in the state with the response from the API request
    *  
13. Testing in React
    *  Objectives
